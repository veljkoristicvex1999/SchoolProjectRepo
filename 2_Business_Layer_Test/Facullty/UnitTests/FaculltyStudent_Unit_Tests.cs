...
namespace _2_Business_Layer_Test.Facullty.UnitTests
{
    public class FaculltyStudent_Unit_Tests
    {
        private readonly Mock<IFacultyStudentService> _sut;

        public FaculltyStudent_Unit_Tests()
        {
            _sut = new Mock<IFacultyStudentService>();
        }
        ...
        {
            var student = MyStudentsList()[0];
            _sut.Setup(s => s.Remove(student)).Verifiable();
            //Act
            _sut.Object.Remove(student);
            //ovo
            //Assert
            _sut.Verify(m => m.Remove(It.Is<FaculltyStudents>(stud => stud.Id == student.Id)));
        }
